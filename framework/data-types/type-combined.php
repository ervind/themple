...
class TPL_Combined extends TPL_Data_Type {
	...
	public $parts_objects	= array();
	public $parts			= array();
	public $js_func			= "set_js_vars";
	public $default			= array( 0 => '' );
	...
	public function __construct( $args ) {
		...
		foreach ( $this->parts as $part ) {
			...
			$this->parts_objects[$part["name"]] = new $type_class( $part );
			$this->parts_objects[$part["name"]]->form_ref_base = $this->name;
			$this->parts_objects[$part["name"]]->is_subitem = true;

		}

		$this->parts = $this->parts_objects;
		unset( $this->parts_objects );
	...
	public function form_field_content () {
		...
		foreach ( $this->parts as $part ) {
			...
			echo '<label for="'. $part->form_ref() .'">' . $part->title . '</label>';
